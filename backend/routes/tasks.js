...
async function assertAssigneeIsProjectMember(projectId, assignedTo) {
...
router.post('/projects/:projectId/tasks', requireProjectAdmin, taskValidation, async (req, res) => {
  ...
  try {
    await assertAssigneeIsProjectMember(req.params.projectId, assigned_to);
    ...
    res.status(err.status || 500).json({ error: err.status ? err.message : 'Failed to create task' });
  }
});
...
router.put('/tasks/:id', taskValidation, async (req, res) => {
  try {
    ...
    await assertAssigneeIsProjectMember(task.project_id, assigned_to);
    ...
    res.status(err.status || 500).json({ error: err.status ? err.message : 'Failed to update task' });
  }
});
